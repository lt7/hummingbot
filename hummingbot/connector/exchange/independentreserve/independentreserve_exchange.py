...
from hummingbot.connector.exchange.independentreserve.independentreserve_api_order_book_data_source import IndependentreserveAPIOrderBookDataSource
from hummingbot.connector.exchange.independentreserve.independentreserve_auth import IndependentreserveAuth
from hummingbot.connector.exchange.independentreserve.independentreserve_order_book_tracker import IndependentreserveOrderBookTracker
from hummingbot.connector.exchange.independentreserve.independentreserve_user_stream_tracker import IndependentreserveUserStreamTracker
...
class IndependentreserveExchange(ExchangeBase):
    ...
    def __init__(self,
                 independentreserve_api_key: str,
                 independentreserve_api_secret: str,
                 trading_pairs: Optional[List[str]] = None,
                 trading_required: bool = True,
                 domain="com"
                 ):
        self._domain = domain
        self._independentreserve_time_synchronizer = TimeSynchronizer()
        super().__init__()
        self._trading_required = trading_required
        self._auth = IndependentreserveAuth(
            api_key=independentreserve_api_key,
            secret_key=independentreserve_api_secret,
            time_provider=self._independentreserve_time_synchronizer)
        self._api_factory = WebAssistantsFactory(auth=self._auth)
        self._rest_assistant = None
        self._throttler = AsyncThrottler(CONSTANTS.RATE_LIMITS)
        self._order_book_tracker = IndependentreserveOrderBookTracker(
            trading_pairs=trading_pairs, domain=domain, api_factory=self._api_factory, throttler=self._throttler)
        self._user_stream_tracker = IndependentreserveUserStreamTracker(
        ...
        self._last_trades_poll_independentreserve_timestamp = 0
    ...
    def name(self) -> str:
        if self._domain == "com":
            ...
            return f"independentreserve_{self._domain}"
    ...
    @property
    def order_book_tracker(self) -> IndependentreserveOrderBookTracker:
        return self._order_book_tracker

    @property
    def user_stream_tracker(self) -> IndependentreserveUserStreamTracker:
    ...
    def status_dict(self) -> Dict[str, bool]:
        ...
        """
        return {
            "symbols_mapping_initialized": IndependentreserveAPIOrderBookDataSource.trading_pair_symbol_map_ready(
                domain=self._domain),
    ...
    @staticmethod
    def independentreserve_order_type(order_type: OrderType) -> str:
        return order_type.name.upper()

    @staticmethod
    def to_hb_order_type(independentreserve_type: str) -> OrderType:
        return OrderType[independentreserve_type]
    ...
    async def cancel_all(self, timeout_seconds: float) -> List[CancellationResult]:
        ...
        try:
            ...
            self.logger().network(
                ...
                exc_info=True,
                app_warning_msg="Failed to cancel order with Independentreserve. Check API key and network connection."
            )
    ...
    async def _create_order(self,
        ...
        price_str = f"{price:f}"
        type_str = IndependentreserveExchange.independentreserve_order_type(order_type)
        side_str = CONSTANTS.SIDE_BUY if trade_type is TradeType.BUY else CONSTANTS.SIDE_SELL
        symbol = await IndependentreserveAPIOrderBookDataSource.exchange_symbol_associated_to_pair(
            trading_pair=trading_pair,
            domain=self._domain,
        ...
        try:
            ...
            self.logger().network(
                f"Error submitting {side_str} {type_str} order to Independentreserve for "
    ...
    async def _status_polling_loop(self):
        ...
        It also updates the time synchronizer. This is necessary because Independentreserve require the time of the client to be
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching account updates.", exc_info=True,
                                      app_warning_msg="Could not fetch account updates from Independentreserve. "
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching trading rules.", exc_info=True,
                                      app_warning_msg="Could not fetch new trading rules from Independentreserve. "
                                                      "Check network connection.")
                await asyncio.sleep(0.5)

    async def get_valid_primary_currency_codes(self):
        primary_currency_codes = await self._api_request(
            method=RESTMethod.GET,
            path_url=CONSTANTS.PRIMARY_CURRENCY_CODES_PATH_URL)
        return primary_currency_codes

    async def get_valid_secondary_currency_codes(self):
        secondary_currency_codes = await self._api_request(
            method=RESTMethod.GET,
            path_url=CONSTANTS.SECONDARY_CURRENCY_CODES_PATH_URL)
        return secondary_currency_codes

    async def get_order_minimum_volumes(self):
        order_minimum_volumes = await self._api_request(
            method=RESTMethod.GET,
            path_url=CONSTANTS.MINIMUM_VOLUMES_PATH_URL)
        return order_minimum_volumes

    async def _update_trading_rules(self):
        primary_currency_codes = await self.get_valid_primary_currency_codes()
        secondary_currency_codes = await self.get_valid_secondary_currency_codes()
        all_currency_codes = list(itertools.product(primary_currency_codes, secondary_currency_codes))
        order_minimum_volumes = await self.get_order_minimum_volumes()

        constructed_exchange_info = []
        for currency_code in all_currency_codes:

            for symbol in CONSTANTS.SYMBOL_DECIMAL_PLACES:
                if symbol["Code"] == currency_code[0]:
                    currency_info = [
                        {
                            "Symbol": currency_code[0] + "-" + currency_code[1],
                            "VolumeDecimalPlaces": symbol["VolumeDecimalPlaces"],
                            "QuoteDecimalPlaces": symbol["QuoteDecimalPlaces"],
                            "OrderMinimumVolume": order_minimum_volumes[currency_code[0]]
                        }
                    ]
                    constructed_exchange_info.append(currency_info)

        trading_rules_list = await self._format_trading_rules(constructed_exchange_info)
    ...
    async def _format_trading_rules(self, exchange_info_dict: Dict[str, Any]) -> List[TradingRule]:
        ...
        for rule in trading_pair_rules:
            try:
                trading_pair = rule[0]["Symbol"]
                min_order_size = rule[0]["OrderMinimumVolume"]
                tick_size = 1 / (10 ** rule[0]["VolumeDecimalPlaces"])
                step_size = rule[0]["OrderMinimumVolume"]
                min_notional = 0  # not provided by IR
        ...
        async for event_message in self._iter_user_event_queue():
            try:
                ...
                if event_type in ("NewOrder", "OrderChanged", "OrderCanceled"):
                    ...
                    client_order_id = event_message["Data"]["OrderGuid"]
                    ...
                    if event_type == "Trade":
                        ...
                        if tracked_order is not None:
                            ...
                            trade_update = TradeUpdate(
                                trade_id=str(event_message["Data"]["TradeGuid"]),
                                client_order_id=client_order_id,
                                exchange_order_id=str(event_message["Data"]["Nonce"]),
                                trading_pair=tracked_order.trading_pair,
                                fee=fee,
                                fill_base_amount=Decimal(event_message["Data"]["Volume"]),
                                fill_quote_amount=Decimal(event_message["Data"]["Volume"]) * Decimal(event_message["Data"]["Price"][0]),
                                fill_price=Decimal(event_message["Data"]["Price"][0]),
                                fill_timestamp=event_message["Data"]["TradeDate"] * 1e-3,
                            )
    ...
    async def _update_order_fills_from_trades(self):
        ...
        in case Independentreserve's user stream events are not working.
        NOTE: It is not required to copy this functionality in other connectors.
        This is separated from _update_order_status which only updates the order status without producing filled
        events, since Independentreserve's get order endpoint does not return trade IDs.
        ...
        if (long_interval_current_tick > long_interval_last_tick
            ...
            # query_time = int(self._last_trades_poll_independentreserve_timestamp * 1e3)
            self._last_trades_poll_independentreserve_timestamp = self._independentreserve_time_synchronizer.time()
            ...
            for trades, trading_pair in zip(results, trading_pairs):
                ...
                for trade in trades["Data"]:
                    exchange_order_id = str(trade["OrderGuid"])
                    if exchange_order_id in order_by_exchange_id_map:
                        ...
                        trade_update = TradeUpdate(
                            trade_id=str(trade["TradeGuid"]),
                            client_order_id=tracked_order.client_order_id,
                            exchange_order_id=exchange_order_id,
                            trading_pair=trading_pair,
                            fee=fee,
                            fill_base_amount=Decimal(trade["VolumeTraded"]),
                            fill_quote_amount=Decimal(trade["quoteQty"]),
                            fill_price=Decimal(trade["Price"]),
                            fill_timestamp=trade["TradeTimestampUtc"] * 1e-3,
                        )
                        self._order_tracker.process_trade_update(trade_update)
                    elif self.is_confirmed_new_order_filled_event(str(trade["TradeGuid"]), exchange_order_id, trading_pair):
                        # This is a fill of an order registered in the DB but not tracked any more
                        self._current_trade_fills.add(TradeFillOrderDetails(
                            market=self.display_name,
                            exchange_trade_id=str(trade["TradeGuid"]),
                            symbol=trading_pair))
                        self.trigger_event(
                            MarketEvent.OrderFilled,
                            OrderFilledEvent(
                                timestamp=float(trade["OrderTimestampUtc"]) * 1e-3,
                                order_id=self._exchange_order_ids.get(str(trade["OrderGuid"]), None),
                                trading_pair=trading_pair,
                                trade_type=TradeType.BUY if trade["OrderType"] else TradeType.SELL,
                                order_type=OrderType.LIMIT_MAKER if "LimitBid" in trade["isMaker"] else OrderType.LIMIT,
                                price=Decimal(trade["Price"]),
                                amount=Decimal(trade["VolumeTraded"]),
    ...
    async def _update_order_status(self):
        # This is intended to be a backup measure to close straggler orders, in case Independentreserve's user stream events
        # are not working.
        # The minimum poll interval for order status is 10 seconds.
        ...
        while True:
            try:
                ...
                self.logger().network(
                    ...
                    exc_info=True,
                    app_warning_msg="Could not fetch user events from Independentreserve. Check API key and network connection."
                )
    ...
    async def _update_balances(self):
        ...
        try:
            ...
            for account in account_info:
                ...
                free_balance = Decimal(account["AvailableBalance"])
                total_balance = Decimal(account["TotalBalance"])
                self._account_available_balances[asset_name] = free_balance
                self._account_balances[asset_name] = total_balance
    ...
    async def _api_request(self,
        ...
        headers = {
            "Content-Type": "application/json" if method == RESTMethod.POST else "application/json"}
        client = await self._get_rest_assistant()

        if is_auth_required:
            url = independentreserve_utils.private_rest_url(path_url, domain=self._domain)
        else:
            url = independentreserve_utils.public_rest_url(path_url, domain=self._domain)
        ...
        async with self._throttler.execute_task(limit_id=path_url):
            ...
            if "code" in parsed_response and "msg" in parsed_response:
                raise IOError(f"The request to Independentreserve failed. Error: {parsed_response}. Request: {request}")
