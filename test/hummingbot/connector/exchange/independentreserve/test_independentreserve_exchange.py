...
from hummingbot.connector.exchange.independentreserve import independentreserve_constants as CONSTANTS, independentreserve_utils
...
from hummingbot.connector.exchange.independentreserve.independentreserve_exchange import IndependentreserveExchange
...
class IndependentreserveExchangeTests(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.exchange = IndependentreserveExchange(
            independentreserve_api_key="testAPIKey",
            independentreserve_api_secret="testSecret",
            trading_pairs=[self.trading_pair],
        )

        self.exchange.logger().setLevel(1)
        self.exchange.logger().addHandler(self)
        self.exchange._independentreserve_time_synchronizer.logger().setLevel(1)
        self.exchange._independentreserve_time_synchronizer.logger().addHandler(self)
        self.exchange._order_tracker.logger().setLevel(1)
        self.exchange._order_tracker.logger().addHandler(self)

        self._initialize_event_loggers()

        IndependentreserveAPIOrderBookDataSource._trading_pair_symbol_map = {
            "com": bidict(
    ...
    def test_check_network_successful(self, mock_api):
        url = independentreserve_utils.public_rest_url(CONSTANTS.PING_PATH_URL)
    ...
    def test_create_order_successfully(self, mock_api):
        ...
        url = independentreserve_utils.private_rest_url(CONSTANTS.ORDER_PATH_URL)
        regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))
        ...
        self.assertEqual(IndependentreserveExchange.independentreserve_order_type(OrderType.LIMIT), request_data["type"])
    ...
    def test_update_balances(self, mock_api):
        url = independentreserve_utils.private_rest_url(CONSTANTS.ACCOUNTS_PATH_URL)
        regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))
        ...
        self.assertEqual(Decimal("10"), available_balances["Xbt"])
        self.assertEqual(Decimal("2000"), available_balances["Ltc"])
        self.assertEqual(Decimal("15"), total_balances["Xbt"])
    ...
    def test_update_order_fills_from_trades_triggers_filled_event(self, mock_api):
        ...
        url = independentreserve_utils.private_rest_url(CONSTANTS.MY_TRADES_PATH_URL)
        regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))
    ...
    def test_update_order_fills_request_parameters(self, mock_api):
        ...
        self.exchange._last_poll_timestamp = (self.exchange.current_timestamp -
                                              self.exchange.UPDATE_ORDER_STATUS_MIN_INTERVAL - 1)
        self.exchange._last_trades_poll_independentreserve_timestamp = 10
        self.async_run_with_timeout(self.exchange._update_order_fills_from_trades())
    ...
    @aioresponses()
    def test_get_primary_currency_code(self, mock_api):
        url = independentreserve_utils.private_rest_url(CONSTANTS.PRIMARY_CURRENCY_CODES_PATH_URL)
        regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))
        ...
        mock_response = primary_currencies
        mock_api.get(regex_url, body=json.dumps(mock_response))

        primary_pairs = self.async_run_with_timeout(self.exchange.get_valid_primary_currency_codes())

        self.assertIn(primary_pairs['btc'], mock_response['Btc'])
        self.assertNotIn(primary_pairs['HBOT'], mock_response['Btc'])
    ...
    @aioresponses()
    def test_update_trading_rules_old(self, mock_api):
        url = independentreserve_utils.private_rest_url(CONSTANTS.EXCHANGE_INFO_PATH_URL)
        regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))
